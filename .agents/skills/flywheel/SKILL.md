...
permissions:
  ...
  forbidden: [Edit, Agent, Skill, WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate, TaskUpdate]
  bash_scope: "find, ls, wc, ao, jq — no git writes"
  write_scope: ".agents/flywheel-status.md only"
  model_tier: micro
context:
  window: fork
  intent:
