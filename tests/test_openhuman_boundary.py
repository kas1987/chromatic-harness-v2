...
async def test_openhuman_readonly_allows_memory_search():
    ...
    # Since there is no real sidecar, this will fail with connection error,
    # but the route_reason should NOT be openhuman_readonly_blocked
    resp = await oh.complete(req)
    assert "openhuman_readonly_blocked" != resp.route_reason
    # It may be openhuman_exception or openhuman_http_error due to missing server
...
async def test_openhuman_write_action_blocked_even_if_not_readonly():
    # Even outside read-only mode, explicit WRITE_ACTIONS are blocked
...
async def test_openhuman_disabled_when_allow_openhuman_false():
    ...
    # Should fallback to mock because openhuman disabled in config
