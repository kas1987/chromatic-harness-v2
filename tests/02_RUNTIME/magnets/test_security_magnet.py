"""Tests for magnets.security_magnet — SecurityMagnet (skeleton)."""
...
from magnets.security_magnet import SecurityMagnet


class TestSecurityMagnetInterface:
    def test_is_base_magnet_subclass(self):
        assert issubclass(SecurityMagnet, BaseMagnet)

    def test_name(self):
        assert SecurityMagnet.name == "security_magnet"

    def test_observe_returns_magnet_event(self):
        event = SecurityMagnet().observe("m1", "post_execution", {})
    ...
    def test_observe_passes_signal_through(self):
        sig = {"scan_result": "clean"}
        event = SecurityMagnet().observe("m1", "post_execution", sig)
        assert event.observed_signal is sig

    def test_observe_sets_magnet_name(self):
        event = SecurityMagnet().observe("m1", "post_execution", {})
        assert event.magnet_name == "security_magnet"

    def test_observe_sets_mission_id(self):
        event = SecurityMagnet().observe("mission-sec", "post_execution", {})
        assert event.mission_id == "mission-sec"

    def test_observe_sets_inflection_point(self):
        event = SecurityMagnet().observe("m1", "validation", {})
        assert event.inflection_point == "validation"
