...
class TestSecretsSurfacePluginNoHits:
    ...
    def test_empty_list_no_hits(self):
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": []})
        assert event.risk_delta == 0.0


class TestSecretsSurfacePluginWithHits:
    def test_one_hit_raises_risk(self):
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": ["AWS_SECRET_KEY"]})
        assert event.risk_delta > 0

    def test_one_hit_recommends_halt(self):
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": ["API_KEY"]})
        assert event.recommended_action == "halt"

    def test_one_hit_negative_confidence(self):
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": ["TOKEN"]})
    ...
    def test_multiple_hits_scale_risk(self):
        one_event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": ["k1"]})
        three_event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": ["k1", "k2", "k3"]})
    ...
    def test_many_hits_risk_capped_at_0_4(self):
        ...
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secrets_detected": hits})
        assert event.risk_delta <= 0.4

    def test_secret_hits_alias(self):
        """secret_hits key should also be recognized."""
        event = SecretsSurfacePlugin().observe("m1", "post_execution", {"secret_hits": ["PASSWORD"]})
        assert event.risk_delta > 0
