...
class TestUserRegisterRequest:
    def test_requires_username_and_password(self) -> None:
        with pytest.raises(ValidationError):
            UserRegisterRequest(password="pw")  # type: ignore[call-arg]  # pragma: allowlist secret
        with pytest.raises(ValidationError):
            UserRegisterRequest(username="u")  # type: ignore[call-arg]

    def test_default_role(self) -> None:
        req = UserRegisterRequest(username="u", password="p")  # pragma: allowlist secret
        assert req.role == "executor"

    def test_custom_role(self) -> None:
        req = UserRegisterRequest(username="u", password="p", role="admin")  # pragma: allowlist secret
...
class TestTokenResponse:
    def test_defaults(self) -> None:
        r = TokenResponse(access_token="tok", user_id="uid", role="executor")  # pragma: allowlist secret
