...
# ---------------------------------------------------------------------------
# Orchestrator.route_to_provider (async)
# ---------------------------------------------------------------------------


class TestOrchestratorRouteToProvider:
    def setup_method(self):
        _ensure_stubs()
    ...
    @pytest.mark.asyncio
    async def test_returns_dict_with_provider_keys(self):
        ...
        mock_router.route.return_value = fake_resp

        with patch("orchestrator.orchestrator.ChromaticRouter", return_value=mock_router):
            result = await orch.route_to_provider(self._mp(), task_type="planning")

        assert result["provider"] == "anthropic"
        assert result["model"] == "claude-sonnet-4-5"
        assert result["reason"] == "best fit"
        assert result["fallback_used"] is False
        assert result["cost_estimate_usd"] == 0.01
        assert isinstance(result["latency_ms"], int)

    @pytest.mark.asyncio
    async def test_passes_task_type_to_router(self):
        from orchestrator.orchestrator import Orchestrator
        ...
        with patch("orchestrator.orchestrator.ChromaticRouter", return_value=mock_router):
            await orch.route_to_provider(self._mp(), task_type="coding")

        call_args = mock_router.route.call_args
        req = call_args[0][0]
        assert req.task_type.value == "coding"

    @pytest.mark.asyncio
    async def test_logs_warnings_and_errors_returned(self):
        from orchestrator.orchestrator import Orchestrator
        ...
        fake_resp = MagicMock(
            ...
            latency_ms=100,
            logs=MagicMock(warnings=["rate_limit_approaching"], errors=[]),
        )
        mock_router = AsyncMock()
        mock_router.route.return_value = fake_resp
        ...
        assert result["warnings"] == ["rate_limit_approaching"]
