...
class TestClassifyText:
    ...
    def test_api_key_indicator_is_p3(self, gate):
        result = gate.classify_text("api_key=abc123")  # pragma: allowlist secret
    ...
    def test_password_indicator_is_p3(self, gate):
        result = gate.classify_text("password=super_secret")  # pragma: allowlist secret
        assert result == PrivacyClass.P3

    def test_bearer_token_is_p3(self, gate):
        result = gate.classify_text("Authorization: bearer mytoken123")  # pragma: allowlist secret
        assert result == PrivacyClass.P3

    def test_aws_secret_is_p3(self, gate):
        result = gate.classify_text("aws_secret_access_key=" + "AK" + "IAIOSFODNN7EXAMPLE")  # pragma: allowlist secret
        assert result == PrivacyClass.P3
    ...
    def test_case_insensitive(self, gate):
        result = gate.classify_text("API_KEY=value")  # pragma: allowlist secret
        assert result == PrivacyClass.P3
