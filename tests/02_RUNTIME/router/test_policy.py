...
class TestProviders:
    ...
    def test_provider_cfg_returns_dict(self, loader_from):
        pl = loader_from(**{"providers.yaml": {"providers": {"gemini": {"enabled": True, "model": "gemini-pro"}}}})
        cfg = pl.provider_cfg("gemini")
        assert cfg["model"] == "gemini-pro"

    def test_provider_cfg_missing_returns_empty(self, loader_from):
        pl = loader_from(**{"providers.yaml": {"providers": {}}})
...
class TestRoutes:
    ...
    def test_route_for_task_existing(self, loader_from):
        pl = loader_from(
            **{"routing-table.yaml": {"routes": {"classification": {"default": "ollama", "fallback": ["mock"]}}}}
        )
    ...
    def test_route_for_task_missing_returns_empty(self, loader_from):
        pl = loader_from(**{"routing-table.yaml": {"routes": {}}})
...
class TestCaching:
    ...
    def test_cache_keyed_by_filename(self, tmp_path):
        (tmp_path / "providers.yaml").write_text(yaml.dump({"providers": {"p1": {}}}), encoding="utf-8")
        (tmp_path / "routing-table.yaml").write_text(yaml.dump({"routes": {"classification": {}}}), encoding="utf-8")
        pl = PolicyLoader(config_dir=tmp_path)
        pl.providers()
        pl.routes()
