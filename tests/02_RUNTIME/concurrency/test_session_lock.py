...
class TestAcquireLock:
    def test_returns_owner_token_string(self, tmp_db):
        token = acquire_lock("mylock", session_id="s1")
        assert isinstance(token, str)
        assert "s1" in token

    def test_token_contains_session_id(self, tmp_db):
        token = acquire_lock("mylock", session_id="session-abc")
    ...
    def test_expired_lock_can_be_reacquired(self, tmp_db):
        ...
        token = acquire_lock("stale-lock", session_id="new-session", timeout_seconds=1.0)
...
class TestReleaseLock:
    def test_release_removes_row(self, tmp_db):
        token = acquire_lock("rel-lock", session_id="s1")
    ...
    def test_released_lock_can_be_reacquired(self, tmp_db):
        token = acquire_lock("reuse", session_id="s1")
        release_lock("reuse", token)
        new_token = acquire_lock("reuse", session_id="s2")
...
class TestSessionLockContextManager:
    ...
    def test_lock_released_on_exit(self, tmp_db):
        ...
        token = acquire_lock("ctx-lock-2", session_id="s2")
