...
class ChromaticRouter:
    ...
    def _register_default_adapters(self):
        ...
        for name, cfg in providers.items():
            if name == "openhuman":
                ...
                # Future: instantiate real adapters here
                # For Phase 1, only openhuman and mock are live
                pass
        # Mock is always available as ultimate fallback
    ...
    def _resolve_provider(self, req: RouteRequest) -> tuple[str, list[str], RouteLogs]:
        """Pick a provider using policy + constraints + preference."""
        logs = RouteLogs()
        ...
        for cand in candidates:
            if cand == "local_vault":
                # local_vault is a conceptual fallback; map to mock for now
                cand = "mock"
            if cand in allowed or cand == "mock":
                # mock always allowed as safe fallback
    ...
    async def route(self, req: RouteRequest | None = None, **kwargs: Any) -> RouteResponse:
        """Execute full routing pipeline: gates → select → adapt → log."""
        if req is None:
