...
def _prune_messages(messages: list[dict[str, Any]], max_turns: int) -> list[dict[str, Any]]:
...
class OllamaRemoteAdapter(BaseAdapter):
    ...
    def __init__(self, name: str, cfg: dict[str, Any]):
        ...
        self.env_key = cfg.get("env_key", "OLLAMA_API_KEY")
    ...
    def _auth_headers(self) -> dict[str, str]:
        token = os.environ.get(self.env_key, "")  # pragma: allowlist secret
        if not token and self.env_key == "OLLAMA_API_KEY":
            token = os.environ.get("OLLAMA_PRO_KEY", "")  # pragma: allowlist secret
        if not token:
            return {}
    ...
    async def health(self) -> AdapterHealth:
        try:
            ...
            start = time.time()
            resp = await client.get(self._url("/api/tags"), headers=self._auth_headers())
    ...
    async def _chat(self, model: str, messages: list[dict[str, Any]]) -> tuple[int, Any]:
        """Call /api/chat and return (status_code, parsed_json or None)."""
        payload = {
            "model": model,
            "messages": messages,
            "stream": False,
        ...
        client = self._get_client()
        resp = await client.post(self._url("/api/chat"), json=payload, headers=self._auth_headers())
        if resp.status_code == 200:
    ...
    async def complete(self, req: RouteRequest) -> RouteResponse:
        ...
        raw_messages = req.input.messages if req.input.messages else [{"role": "user", "content": req.objective}]
        messages = _prune_messages(raw_messages, self.max_history_turns)

        models = [self.model] + list(self.cfg.get("fallback_models", []))
        last_status = 0
        last_error_body: Any = None
        start = time.time()
        for model in models:
            try:
                status, body = await self._chat(model, messages)
            except Exception as e:
                logs.errors.append(f"Ollama error for {model}: {str(e)}")
                return self.normalize_error(req.request_id, f"Ollama model {model} unavailable: {e}")
            last_status = status
            last_error_body = body
            if status == 200:
                latency_ms = int((time.time() - start) * 1000)
                content = body.get("message", {}).get("content", "") if body else ""
        ...
        # All model choices failed.
        if last_status == 404:
            detail = last_error_body.get("error", "") if isinstance(last_error_body, dict) else ""
            message = "Ollama model unavailable (status 404)"
            if detail:
                message += f": {detail}"
        else:
            message = f"Ollama status {last_status}"
        return self.normalize_error(req.request_id, message)
