"""CAT adapter — routes missions to Chromatic Atomic Tower via HTTP."""
...
class CATAdapter(BaseAdapter):
    """Routes missions to Chromatic Atomic Tower via HTTP wrapper."""
    ...
    async def complete(self, req: RouteRequest) -> RouteResponse:
        ...
        try:
            ...
            if response.status_code != 200:
                return self.normalize_error(
                    req.request_id,
                    f"CAT HTTP status {response.status_code}: {response.text[:200]}",
                )
            ...
            logs.errors.append(f"CAT adapter error: {str(e)}")
