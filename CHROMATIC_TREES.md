...
>
> **Status:** canonical · **Maintained by:** harness agents · **Last reconciled:** 2026-06-03 (canon-sync: review-intake paths, legacy `10_RUNTIME` removal)
>
...
| 07 | `07_LOGS_AND_AUDIT/` | Execution/decision/trace logs, governance intelligence, pre_session manifests; **review intake** under `review_intake/` | CANONICAL (append-only) |
...
| 10 | `10_RUNTIME/` | Deprecated logs-only stub (removed 2026-06-03) — use `02_RUNTIME/` | RETIRED |
...
├── 07_LOGS_AND_AUDIT/          CANONICAL  execution · decisions · traces · pre_session · review_intake/
├── 08_PDRS/                    CANONICAL  design records + _PDR_TEMPLATE.md
├── 09_DEPLOYMENT/              CANONICAL  deploy · routing policy · CI assets
├── 10_RUNTIME/                 RETIRED    removed 2026-06-03 (was logs-only stub → §4)
...
|--------------------|----------------|-----------|--------|
| `10_RUNTIME/` (logs stub) | `02_RUNTIME/` | canon-sync 2026-06-03 | ✅ removed |
| `02_RUNTIME/10_RUNTIME/` (logs stub) | `02_RUNTIME/` | canon-sync 2026-06-03 | ✅ removed |
| `00_PLANNING/` *(PDR zip only)* | `07_LOGS_AND_AUDIT/review_intake/` | never create in harness | N/A |
| `02_LOGS/` *(PDR zip only)* | `07_LOGS_AND_AUDIT/review_intake/` | never create in harness | N/A |
| `03_PLAYBOOKS/` *(PDR zip only)* | `04_PLAYBOOKS/` | never create in harness | N/A |
| `02_DOCS/` | `docs/` | V3-6 (`8lri.2`) | ⚠️ deprecated — prefer `docs/` |
| `agent_handoffs/` | `12_HANDOFFS/` + `.agents/handoffs/` | V3-6 | ⚠️ migrate machine handoffs to `.agents/handoffs/` |
| `reports/` | `05_REPORTS/` | V3-6 | ✅ retired |
| `queue/`, `issues/` | `07_LOGS_AND_AUDIT/` | V3-6 | ✅ retired |
| `dashboards/` | `09_DEPLOYMENT/dashboards/` | V3-6 | ✅ retired |
| `hooks/` | `git_hooks/` | V3-6 | ✅ retired |
| `state/` (lease ledger) | `01_STATE/leases/` | V3-6 | ✅ retired |

**Do not** unpack portable PDR bundles using zip paths (`00_PLANNING/`, `02_LOGS/`, `03_PLAYBOOKS/`).
Use the mapping in [`07_LOGS_AND_AUDIT/audits/repo_reorg_audit_2026-06-01.md`](07_LOGS_AND_AUDIT/audits/repo_reorg_audit_2026-06-01.md).

Write only to the **canonical home** (right column). Some legacy root dirs may still exist on disk
during transition — do not add new files there.
...
| **Review-intake queue / findings** | `07_LOGS_AND_AUDIT/review_intake/` (`queue.json`, `findings.jsonl`) | PDR: `08_PDRS/PDR_REVIEW_INTAKE_2026-06-01.md` |
| **Machine session handoff** | `.agents/handoffs/latest.json` | not `00_PLANNING/` |
| **Human session handoff** | `12_HANDOFFS/` · `SESSION_COMPACT.md` | — |
