...
|---|---|---|
| C1 mechanical (config edit, format, scaffold, fixture) | `haiku` (local/OL only for NON-code transforms) | ~10x cheaper; zero judgment |
| C2 structured (single-file code edit, code review, smoke test) | `sonnet` | haiku has no extended thinking → misses subtle correctness bugs in review/edits (revised 2026-06-05) |
...
> **Code floor (2026-06-05):** any code-touching task (edit, review, refactor) has a
> Claude floor — never route it to a local/open model (3B–14B). Reserve T0–T2
> open models for non-code transforms only (format, extract, classify, summarize).
