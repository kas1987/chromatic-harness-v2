...
### HIGH: Duplicate PreToolUse Agent hooks

2 Agent gate hooks configured ù may run gate.py twice per agent dispatch.
...
| claude_code | `settings.json` | PreToolUse | Agent | 5 | `python C:/Users/kas41/chromatic-harness-v2/02_RUNTIME/router/gate.py` |
...
| claude_code | `settings.json` | SessionStart | (all) | None | `python scripts/session_start.py` |
| claude_code | `settings.json` | PreCompact | (all) | None | `bd prime` |
| claude_code | `settings.json` | PreToolUse | Agent | None | `python 02_RUNTIME/router/gate.py` |
