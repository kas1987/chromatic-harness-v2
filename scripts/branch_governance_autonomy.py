...
def _workflow_not_found(result: dict[str, Any]) -> bool:
    ...
    return ("workflow" in stderr and "not found" in stderr) or ("could not find any workflows named" in stderr)
...
def _mode_cloud(*, workflow: str, include_ci_weekly: bool) -> dict[str, Any]:
    ...
    ok = selected_ok and all(
        r.get("ok") for r in results[-1:] if r.get("cmd", [None, None, None])[-1] == "ci-governance-weekly.yml"
    )
