...
def main():
    ...
    return subprocess.call([sys.executable, str(root/'scripts/log_harness_event.py'),'--repo-root',str(root),'--event-type','status_update','--severity','info','--category','manual_note','--status',args.status,'--surface','manual','--raw-excerpt',f'Status update for {args.event_id}','--linked-fix',args.linked_fix])
