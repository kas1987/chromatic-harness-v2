...
Reads new entries from ~/.agents/ol_queue/intake_queue.jsonl (or C:\chromatic-harness-v2\07_LOGS_AND_AUDIT\intake_queue.jsonl)
...
def read_intake_queue(path=None):
    ...
    if not path:
        path = Path("C:/chromatic-harness-v2/07_LOGS_AND_AUDIT/intake_queue.jsonl")
...
def validate_bead(bead_id, cat_root=None):
    ...
    try:
        result = subprocess.run(
            ...
            capture_output=True, text=True, cwd=str(cat_root)
        )
    ...
    for entry in entries:
        ...
        if success:
            ...
            if not args.dry_run:
                ...
                if not valid:
                    print(f"[cat-intake-bridge] WARNING: BEAD validation failed for {bead['id']}: {msg}", file=sys.stderr)
        else:
