...
def main() -> int:
    ...
    parser.add_argument("--ledger", default="state/leases/active_leases.jsonl")
            ...
            for lres in left.get("resources", []):
                for rres in right.get("resources", []):
                    if overlaps(lres, rres):
                        collisions.append({"left": left.get("lease_id"), "right": right.get("lease_id"), "resource_a": lres, "resource_b": rres})
